...
def main():
    ...
    test_features = prepare_features_for_symbols(load_historical_data(symbols, "2023-01-01", "2023-12-31"))

    # Create and train LSTM strategy
    ml_strategy = MLStrategy(FEATURE_NAMES, symbols)
    
    # Train with validation
    ml_strategy.train(train_features)
    ...
    plot_curves(
        ...
        leverage=ml_stats['leverage_curve'],
        leverage_component=ml_stats['leverage_components'],
        title="ML Strategy vs Equal Weight Benchmark"
    )
    plt.savefig('results/equity_curves.png', bbox_inches='tight', dpi=300)
    
    print(ml_stats['leverage_components'])
    ...
    ml_stats['leverage_components'].to_csv('results/ml_stats.csv', index=False)
