...
class MLStrategy(Strategy):
    ...
    def should_rebalance(self, timestep: int) -> bool:
        ...
        # target_weights = self.predict_weights(self.current_data)
        
        # max_deviation = max(abs(current_weights[s] - target_weights[s]) 
        #                   for s in self.symbols)
        
        # # Update adaptive threshold based on recent performance
        # if len(self.prediction_history) > 20:
        #     recent_accuracy = self._calculate_prediction_accuracy()
        #     self.adaptive_threshold = max(0.05, min(0.15, 0.1 * (1 - recent_accuracy)))
        
        # return max_deviation > self.adaptive_threshold
