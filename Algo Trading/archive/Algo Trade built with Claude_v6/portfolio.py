...
class Portfolio:
    ...
    def get_weights(self, current_price: dict[str, float]) -> dict[str, float]:
        """
        Get the current weights of the portfolio
        """
        return {symbol: asset.position * current_price[symbol] / self.get_total_value(current_price) for symbol, asset in self.assets.items()}