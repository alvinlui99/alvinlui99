...
from config import (DATA_PATH, DATA_TIMEFRAME, COMMISSION_RATE, MIN_CASH,
                   TRADING_DAYS, PERIODS_PER_DAY)
...
class Backtester:
    ...
    def _place_trade(self, symbol: str, size: float, price: float) -> None:
        """Record a trade with commission"""
        trade_cost = size * price
        commission_cost = abs(trade_cost * self.commission)
        
        if self.portfolio.cash - trade_cost - commission_cost < MIN_CASH:
            return
    ...
    def _calculate_stats(self) -> TradeStats:
        ...
        stats = TradeStats(
            ...
            sharpe_ratio=np.sqrt(TRADING_DAYS * PERIODS_PER_DAY) * returns.mean() / returns.std() if len(returns) > 1 else 0,
    ...
    def _calculate_win_rate(self, trades_df: pd.DataFrame) -> Dict[str, float]:
        ...
        # Calculate trade PnL
        trades_df['trade_value'] = trades_df['position'] * trades_df['price']
    ...
    def _calculate_sharpe_ratio(self, returns: pd.Series) -> float:
        ...
        periods_per_year = TRADING_DAYS * PERIODS_PER_DAY  # For hourly data
        
        excess_returns = returns - (risk_free_rate / periods_per_year)
        
        # Annualize returns and volatility
        annualized_returns = excess_returns.mean() * periods_per_year
