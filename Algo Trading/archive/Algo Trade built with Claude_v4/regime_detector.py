...
class RegimeDetector:
    ...
    def get_regime_leverage(self, data: Dict[str, pd.DataFrame], weights: Dict[str, float],
        ...
        # More conservative base leverage calculation
        ...
        # Stricter volatility adjustment
        vol_percentile = self._calculate_vol_percentile(volatility)
        vol_factor = 1.0 - (vol_percentile * self.vol_adj_factor)
        ...
        # Calculate momentum with stricter conditions
    ...
    def _calculate_drawdown_factor(self, returns: float) -> float:
        """Calculate drawdown factor with more aggressive reduction"""
        if returns >= 0:
            return 1.0
        
        # More aggressive exponential reduction
        return np.exp(2 * returns)  # Changed from 2 to 3 for faster reduction

    def _calculate_momentum_factor(self, momentum: float, vol_percentile: float, confidence: float) -> float:
        """Calculate momentum factor with even stricter conditions"""
        momentum_factor = 1.0
        
        # Only increase leverage if all conditions are met with stricter thresholds
        if (momentum > 0.8 and          # Very strong momentum (90%+ of signals positive)
            vol_percentile < 0.2 and    # Very low volatility (bottom 20%)
            confidence > 0.8):          # Very high confidence (90%+)
            
            # More conservative boost calculation
            boost = min(
                self.momentum_adj_factor,         # Maximum 20% boost
                1.0 + (
                    (momentum - 0.8) * 2 +        # Momentum contribution
                    (0.2 - vol_percentile) +      # Volatility contribution
                    (confidence - 0.8)            # Confidence contribution
                )
            )
