...
class MLStrategy(Strategy):
    ...
    def train(self, train_data: Dict[str, pd.DataFrame], val_data: Dict[str, pd.DataFrame]) -> Dict[str, float]:
        ...
        # Configure base model with parameters encouraging more active trading
        base_model = lgb.LGBMRegressor(
            n_estimators=100,
            learning_rate=0.1,
            num_leaves=31,
    ...
    def should_rebalance(self, portfolio: Portfolio, target_weights: Dict[str, float], 
                        current_prices: Dict[str, float], timestep: int) -> bool:
    ...
    def __call__(self, portfolio: Portfolio, current_prices: Dict[str, dict], 
        ...
        if not self.should_rebalance(portfolio, weights, current_prices, timestep):
        ...
        for symbol, weight in weights.items():
            ...
            if weight > 0:
                target_positions[symbol] = (weight * adjusted_equity) / price
            else:
