...
class RegimeDetector:
    ...
    def get_regime_leverage(self, data: Dict[str, pd.DataFrame], weights: Dict[str, float],
                            equity_curve: pd.Series, timestep: int) -> float:
        """
        ...
        if regime > 0:
            ...
            regime_factor = 2.0 + (self.regime_adj_factor * 6.0 * (regime + 1))  
            trend_factor = 1.0 + (self.trend_adj_factor * 4.0 * (trend_score + 1))
        else:
        ...
        if regime > 0:
            vol_factor = 1.0 - (vol_percentile * self.vol_adj_factor * 0.2)
        else:
            vol_factor = 1.0 - (vol_percentile * self.vol_adj_factor * 1.2)
        ...
        if regime > 0:
            return max(1.0, min(5.0, adjusted_leverage))
        else:
            return max(1.0, min(2.5, adjusted_leverage))
    ...
    def _determine_regime_with_hysteresis(self, trend_score: float, confidence: float) -> int:
        ...
        if abs(trend_score) > 0.3:
            confidence_threshold = 0.25  # Lower confidence needed for strong trends
        else:
    ...
    def _calculate_momentum_factor(self, momentum: float, vol_percentile: float, confidence: float) -> float:
        ...
        if momentum > 0.2:  # Even lower threshold
            if vol_percentile < 0.6 and confidence > 0.4:  # More relaxed conditions
                # More aggressive boost calculation
                boost = min(
                    4.0,  # Maximum 100% boost
                    1.0 + (
                        (momentum - 0.2) * 5.0 +     # Much stronger momentum contribution
                        (0.6 - vol_percentile) * 3.0 + # Stronger volatility contribution
                        (confidence - 0.4) * 3.0     # Stronger confidence contribution
                    )
                )
