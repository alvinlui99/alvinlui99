...
class Trader:
    ...
    def get_qty(self, prices1: pd.Series, prices2: pd.Series, price1: float, price2: float, margin_balance: float) -> float:
        ...
        while True:
            ...
            for pair in self.selected_pairs:
                ...
                if pnl < position['margin_balance'] * self.config.stop_loss_pc:
                    if position['qty1'] > 0:
                        self.client.new_order(symbol=c1, side='SELL', quantity=abs(position['qty1']), type='MARKET')
                        self.client.new_order(symbol=c2, side='BUY', quantity=abs(position['qty2']), type='MARKET')
                        positions.loc[positions['pair'] == f'{c1}-{c2}', 'stop_loss'] = 1
                    else:
                        self.client.new_order(symbol=c1, side='BUY', quantity=abs(position['qty1']), type='MARKET')
                        self.client.new_order(symbol=c2, side='SELL', quantity=abs(position['qty2']), type='MARKET')
                        positions.loc[positions['pair'] == f'{c1}-{c2}', 'stop_loss'] = -1
                ...
                if h1 > self.config.long_threshold and position['stop_loss'] != 1 and position['side'] == 0:
                    qty1, qty2 = self.get_qty(price_history1.iloc[-self.config.ewm_window:],
                    ...
                    self.client.new_order(symbol=c1, side='BUY', quantity=abs(qty1), type='MARKET')
                    self.client.new_order(symbol=c2, side='SELL', quantity=abs(qty2), type='MARKET')
                    ...
                    positions.loc[positions['pair'] == f'{c1}-{c2}', 'stop_loss'] = stop_loss
                    positions.loc[positions['pair'] == f'{c1}-{c2}', 'side'] = side
                    self.client.new_order(symbol=c1, side='SELL', quantity=abs(qty1), type='MARKET')
                    self.client.new_order(symbol=c2, side='BUY', quantity=abs(qty2), type='MARKET')
