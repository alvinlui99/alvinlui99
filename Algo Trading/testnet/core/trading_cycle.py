...
class TradingCycle:
    ...
    def _prepare_leverage_strategy(self) -> LeverageStrategy:
        self.leverage_strategy = RegimeLeverageStrategy()
        self.leverage_strategy.configure()

    def _prepare_trading_strategy(self) -> TradingStrategy:
        self.trading_strategy = MLStrategy()
        self.trading_strategy.configure(
            features=FeatureConfig.FEATURE_NAMES,
            symbols=TradingConfig.SYMBOLS,
            model=self.model,
            leverage_strategy=self.leverage_strategy
        )
    ...
    def _setup_trading_cycle(self):
        ...
        if self.model is None:
            self._prepare_model()
        if self.leverage_strategy is None:
            self._prepare_leverage_strategy()
